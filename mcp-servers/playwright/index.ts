...
const SERVER_NAME = "opencode-browser-agent";
...
async function clickElement(selector: string, index = 0) {
  const p = await ensureBrowser();
...
async function clickWithIndex(index: number, selector: string | null = null) {
  const p = await ensureBrowser();
...
async function fillForm(selector: string, value: string) {
  const p = await ensureBrowser();
...
async function extractContent(selector: string | null = null) {
  const p = await ensureBrowser();
...
async function takeScreenshot(filename: string | null = null) {
  const p = await ensureBrowser();
...
async function scrollPage(direction: string) {
  const p = await ensureBrowser();
...
async function main() {
  ...
  console.error("OpenCode Browser Agent MCP Server started");
